...
1. int registerStudents(int noOfStudents, Student** arrStudents)
    Input: noOfStudents (initially 0), arrStudents (array of Student pointers)
    ...
    Output: Returns total number of students, Fills arrStudents array
...
1. void printFinalReport(Student* arrStudents[], int noOfStudents)
    ...
    Output:
        Displays final report on console
...
2. void printArrOfStudent(Student* arrStudents[], int noOfStudents)
    ...
    Output: Prints student rows and summary

3. double printStudent(Student* student)
    Input: student
    Precondition: Student pointer must not be NULL

    Logic:
        Print marks
        Compute total, percentage, grade

    Output: Returns percentage of that student

### Module 4: Main Module
Module Name: Main
Purpose:
Controls program flow and coordinates all modules.

Function:
1. int main()
    Input: None (uses input file)
    Precondition: All modules compiled and linked correctly
